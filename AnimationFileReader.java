...
public class AnimationFileReader {
    ...
    while (sc.hasNext()) {
      ...
      switch (command) {
        ...
        case "radians":
          ...
          builder.addRotationToChange(rotateByInfo.name,
                  rotateByInfo.getFromRadRotation(),
                  rotateByInfo.getToRadRotation(),
  ...
  private RotateByInfo readRotateByInfo(Scanner sc) throws
    ...
    while (!info.isAllInitialized()) {
      ...
      switch (command) {
        case "rotateto":
          info.setFromRadRotation(sc.nextFloat());
          info.setToRadRotation(sc.nextFloat());
          break;
        case "name":
          info.setName(sc.next());
          break;
  ...
  class ShapeInfo extends Inputable {
    ...
    void setRadians(float radians) {
      ...
      valueFlags.remove("radians", true);
    }
  ...
  class RotateByInfo extends Inputable {
    ...
    RotateByInfo() {
      ...
      valueFlags.put("name", false);
      valueFlags.put("toRad", false);
      valueFlags.put("fromRad", false);
      valueFlags.put("start", false);
      valueFlags.put("end", false);
    ...
    void setFromRadRotation(float fromRad) {
      this.fromRad = fromRad;
      valueFlags.replace("fromRad", true);
    }

    void setToRadRotation(float toRad) {
      this.toRad = toRad;
      valueFlags.replace("toRad", true);
    }
    ...
    float getFromRadRotation() {
      return fromRad;
    }

    float getToRadRotation() {
      return toRad;
    }
